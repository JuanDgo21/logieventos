...
  
  // ✅ CORRECCIÓN (S2933): 'subscriptions' nunca se reasigna
...
  constructor(
    // ✅ CORRECCIÓN (S2933): 'eventService' nunca se reasigna
    private readonly eventService: EventService,
    // ✅ CORRECCIÓN (S2933): 'fb' nunca se reasigna
    private readonly fb: FormBuilder,
    // ✅ CORRECCIÓN (S2933): 'authService' nunca se reasigna
...
        const usedEventTypeIds = new Set(
          // ✅ CORRECCIÓN (S4325): Aserción 'as EventType' innecesaria
          this.events.map(event => typeof event.eventType === 'object' ? (event.eventType)._id : event.eventType)
        );
...
        [event.name, event.description, event.location].some(
          // ✅ CORRECCIÓN (S6502): Usar optional chaining (?.)
...
        (typeof event.eventType === 'object' ? 
          // ✅ CORRECCIÓN (S4325): Aserción 'as EventType' innecesaria
          (event.eventType)._id === this.selectedCategory : 
...
          ...event,
          // ✅ CORRECCIÓN (S4325): Aserción 'as EventType' innecesaria
          eventType: event.eventType && typeof event.eventType === 'object' ? (event.eventType)._id : event.eventType,
...
    const d = new Date(date);
    // ✅ CORRECCIÓN (S7773 - similar): Reemplazado isNaN por Number.isNaN

...
// Controlador especial: Asignar proveedor a recurso (versión corregida)
exports.assignProviderToResource = async (req, res) => {
    try {
        ...
        // Verificar que el índice del recurso sea válido
        if (resourceIndex >= event.requiredResources.length) {
            return res.status(400).json({ message: 'Índice de recurso no válido' });
        }

        // Actualizar el proveedor asignado
        ...
        res.status(500).json({ 
            message: 'Error al asignar proveedor al recurso', 
            error: error.message 
        });
    }
};
